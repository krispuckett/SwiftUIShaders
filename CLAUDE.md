...
- `Sources/SwiftUIShaders/Shaders/SwiftUIShaders.metal`: 41 `[[ stitchable ]]` SwiftUI fragment shaders (source of truth) + shared helpers.
- `Sources/SwiftUIShaders/ShaderEffects.swift`: one typed `View` modifier per shader, plus the two `ViewModifier`s that do the `layerEffect` plumbing.
